...
public class ItemIT {
  ...
  public void testGetItemDetail() throws Exception {
    MvcResult result = mockMvc.perform(get("/items/{id}", item.getItemId())
            .with(user(buyer)))
        .andExpect(status().isOk())
        .andReturn();
  ...
  public void testUpdateItem() throws Exception {
    ...
    MvcResult result = mockMvc.perform(put("/items/{itemId}", item.getItemId())
            .with(user(seller))
            .contentType(MediaType.APPLICATION_JSON)
            .content(requestBody))
        .andExpect(status().isOk())
  ...
  public void testUpdateStatus() throws Exception {
    ...
    MvcResult result = mockMvc.perform(put("/items/{itemId}/status", item.getItemId())
            .with(user(seller))
            .contentType(MediaType.APPLICATION_JSON)
            .content(requestBody))
        .andExpect(status().isOk())
  ...
  public void testDeleteItem() throws Exception {
    mockMvc.perform(delete("/items/{itemId}", item.getItemId())
            .with(user(seller)))
        .andExpect(status().isNoContent());

    mockMvc.perform(get("/items/{id}", item.getItemId())
            .with(user(buyer)))
  ...
  public void testGetOwnItems() throws Exception {
    ...
    MvcResult result = mockMvc.perform(get("/items/me")
            .with(user(seller))
            .param("page", "0")
            .param("size", "10")
            .param("sortField", "publishedAt")
