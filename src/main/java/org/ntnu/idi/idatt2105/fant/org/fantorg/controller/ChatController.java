 ...
 *
 * <p>Provides endpoints for retrieving user chats, fetching messages for a specific chat, and
 * obtaining chat profile details. Also contains a WebSocket endpoint to process and route chat
 * messages.
 */
@RestController
@Slf4j
...
public class ChatController {
  ...
  @Operation(
      ...
      description =
          "Retrieves a paginated list of chat conversations for the authenticated user, sorted by the specified field and direction.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  public ResponseEntity<Page<ChatDto>> getUserChats(
      ...
      @Parameter(description = "sorting field") @RequestParam(defaultValue = "lastEntry")
          String sortField,
  ...
  @Operation(
      ...
      description =
          "Retrieves a paginated list of chat messages between the authenticated user and the specified recipient for a given item.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  public ResponseEntity<Page<ChatMessageDto>> findChatMessages(
    ...
    Page<ChatMessageDto> messages =
        chatMessageService.findChatMessages(user.getEmail(), recipientId, itemId, pageable);
   ...
   *
   * <p>This method receives a chat message, persists it, and then sends a chat notification to the
   * recipient. Note: This endpoint is not exposed as a REST API but rather as a WebSocket message
   * handler.
   *
  ...
  public ChatProfileDto getRecipientInfo(
      @Parameter(description = "Recipient mail") @PathVariable String recipientId,
  ...
  @Operation(
      summary = "Send chat",
      description =
          "Sends a chat to a seller, creating a new room if it does not exist, else uses the existing chat room")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
