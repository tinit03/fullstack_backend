...
/**
 * Service implementation for managing items, including creation, editing, deletion, retrieval, and
 * search with filtering and faceting. This service also handles image uploads to Cloudinary,
 * bookmark logic, and location lookup via BringService.
 */
@Service
...
public class ItemServiceImpl implements ItemService {
  ...
  /**
   * Creates a new item based on the provided DTO and seller info.
   *
   * @param dto The data for the new item.
  ...
  public Item updateItem(Long id, ItemEditDto updatedItem, User seller) {
    ...
    Category subCategory =
        categoryRepository
            .findById(updatedItem.getSubcategoryId())
    ...
    List<Image> imagesToDelete =
        existingImages.stream()
            .filter(
                img -> img.getPublicId() != null && !updatedPublicIds.contains(img.getPublicId()))
            .toList();
  ...
  /**
   * Changes the status of an item (e.g., ACTIVE, SOLD, INACTIVE).
   *
   * @param id The ID of the item.
   * @param status The new status.
   * @param seller The item owner.
  ...
  /**
   * Deletes an item and its images from Cloudinary.
   *
   * @param id The ID of the item.
   * @param seller The user deleting the item.
   */
  @Override
  ...
  /**
   * Retrieves an item and adds bookmark/ownership flags to the DTO.
   *
   * @param id The ID of the item.
   * @param user The requesting user.
   * @return The {@link ItemDto} with status flags.
   */
  @Override
  ...
  /**
   * Retrieves all public items, optionally filtered by status and excluding the current user's
   * listings.
   *
   * @param pageable Pagination info.
   * @param status The status to filter by.
   * @param user The requesting user.
   * @return A page of {@link ItemDto}.
   */
  @Override
  ...
  /**
   * Searches items using various filters and returns matching items along with facet counts.
   *
   * @param filter The filtering and search criteria.
   * @param pageable Pagination info.
   * @param user The requesting user.
   * @return A {@link ItemSearchResponse} containing results and facet metadata.
   */
  @Override
  public ItemSearchResponse searchItems(ItemSearchFilter filter, Pageable pageable, User user) {
    Specification<Item> baseSpec = buildItemSpec(filter, null);
    Specification<Item> fullSpec =
        (user != null) ? baseSpec.and(ItemSpecification.hasNotSeller(user)) : baseSpec;
    ...
    response.setConditionFacet(
        facetUtil.getEnumFacetCounts(specForCondition, "condition", Condition.class));
    ...
    response.setCategoryFacet(
        facetUtil.getLongFacetCounts(specForCategory, "subCategory.parentCategory.id"));
    response.setSubCategoryFacet(
        facetUtil.getLongFacetCounts(specForSubCategory, "subCategory.id"));
  ...
  /**
   * Retrieves items listed by a specific seller.
   *
  ...
  /**
   * Marks each item in the page with bookmark status for the given user.
   *
   * @param dtos A page of item DTOs.
   * @param user The user to check bookmarks against.
   * @return Updated page with bookmark flags set.
   */
  private Page<ItemDto> markDtosWithBookmarkStatus(Page<ItemDto> dtos, User user) {
    ...
    Set<Long> bookmarkedIds =
        bookmarkRepository.findByUser(user).stream()
  ...
  /**
   * Builds a specification for filtering items based on the given filter.
   *
   * @param filter The filtering criteria.
   * @param excludeField Optional field to exclude from the filter.
   * @return The {@link Specification} representing the filter.
   */
  private Specification<Item> buildItemSpec(ItemSearchFilter filter, String excludeField) {
    ...
    if (!"keyword".equals(excludeField)
    ...
    if (!"category".equals(excludeField)
        && filter.getCategoryId() != null
        && !filter.getCategoryId().isBlank()) {
      List<Long> categoryIds =
          Arrays.stream(filter.getCategoryId().split(","))
              .map(String::trim)
              .filter(s -> !s.isBlank())
              .map(Long::parseLong)
              .toList();
    ...
    if (!"subCategory".equals(excludeField)
        && filter.getSubCategoryId() != null
        && !filter.getSubCategoryId().isBlank()) {
      List<Long> subCategoryIds =
          Arrays.stream(filter.getSubCategoryId().split(","))
              .map(String::trim)
              .filter(s -> !s.isBlank())
              .map(Long::parseLong)
              .toList();
    ...
    if (!"condition".equals(excludeField)
        && filter.getCondition() != null
        && !filter.getCondition().isBlank()) {
      List<Condition> conditions =
          Arrays.stream(filter.getCondition().split(","))
              .map(String::trim)
              .filter(s -> !s.isBlank())
    ...
    if (!"county".equals(excludeField)
        && filter.getCounty() != null
        && !filter.getCounty().isBlank()) {
      List<String> counties =
          Arrays.stream(filter.getCounty().split(","))
              .map(String::trim)
              .filter(s -> !s.isBlank())
              .toList();
    ...
    if (!"price".equals(excludeField)) {
      BigDecimal min =
          filter.getMinPrice() != null ? BigDecimal.valueOf(filter.getMinPrice()) : null;
      BigDecimal max =
          filter.getMaxPrice() != null ? BigDecimal.valueOf(filter.getMaxPrice()) : null;
