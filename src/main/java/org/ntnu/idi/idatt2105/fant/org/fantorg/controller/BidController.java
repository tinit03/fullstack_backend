...
/**
 * REST controller for managing bid operations. Provides endpoints for placing bids, retrieving bids
 * for a specific item or by bid id, and accepting or rejecting bids.
 */
@RestController
@RequestMapping("/bids")
...
public class BidController {
  ...
  public ResponseEntity<List<BidDto>> getBidsForItem(
      @Parameter(description = "Identification of item") @PathVariable Long itemId) {
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Bid accepted and order created successfully",
            content = {
              @Content(
                  mediaType = "application/json",
   ...
   * @return a ResponseEntity containing the order details resulting from the accepted bid
   */
  @Operation(summary = "Reject Bid", description = "Rejects bid")
  @ApiResponses(
      value = {
