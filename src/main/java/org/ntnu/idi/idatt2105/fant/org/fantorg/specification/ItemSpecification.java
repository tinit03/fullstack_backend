...
/**
 * Specification class for filtering and querying items based on various conditions. This class
 * provides static methods that return Specifications for different criteria related to the Item
 * entity, allowing for flexible querying.
 *
 * <p>Each method in this class returns a Specification Item that can be used to create dynamic
 * queries with different conditions such as price ranges, categories, status, location, etc. These
 * Specifications can be combined and used in repository queries.
 *
 * @version 1.0
 */
public class ItemSpecification {
  ...
  /**
   * Returns a Specification to filter items based on a keyword found in any of their fields (title,
   * description, or tags).
   *
  ...
  public static Specification<Item> hasSubCategoryIn(List<Long> subCategoryIds) {
    return (root, query, cb) -> root.get("subCategory").get("id").in(subCategoryIds);
  }
  ...
  public static Specification<Item> isInCounties(List<String> counties) {
    return (root, query, cb) -> {
      List<String> lowered =
          counties.stream().map(String::toLowerCase).collect(Collectors.toList());
